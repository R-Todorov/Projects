...
public class MetricTable extends JTable 
{
  ...
  public boolean validInput(String fromDate, String endDate, MetricFilter filter){
      ...
      Date currFromDate, currToDate, end, start;
		...
		try {
			...
			currFromDate = (Date) format.parseObject(fromDate);
			currToDate = (Date) format.parseObject(endDate);
          ...
          if(currFromDate.after(end) || currFromDate.before(start) || currToDate.after(end) || currToDate.before(start)) {
          	...
          	if(currFromDate.after(end) || currFromDate.before(start)){
          		fromDate = storage.getStartDate().substring(0,10);
          		filter.startDate.setText(fromDate);
          	}
          	if(currToDate.after(end) || currToDate.before(start)){
          		endDate = storage.getEndDate().substring(0,10);
          		filter.endDate.setText(endDate);
          	}
          	return false;
          } else {
          	return true;
